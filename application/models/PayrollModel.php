...
class PayrollModel extends CI_Model
{
    ...
    public function generate_payroll($startDate, $endDate, $cutoff)
    {
        ...
        // Fetch assigned personnel for the cutoff / settings
        $personnel = $this->db
            ->select('
            ppa.personnelID,
            p.first_name,
        ...
        foreach ($personnel as $person) {
            ...
            $rateTypeRaw  = $person->rateType;                 // e.g. "Day", "Hour", "Bi-Month"
            $rateType     = strtolower(trim($rateTypeRaw));    // normalize

            $days      = 0;
            $gross     = 0;
            $hoursPerDay = 8; // your system assumes 8 hours = 1 day

            // 🧮 Rate type logic aligned with your "correct payroll" sample
            if (in_array($rateType, ['day', 'daily'])) {
                // Clark: 24 hours / 8 = 3 days → 3 * 150 = 450
                $days  = $hoursPerDay > 0 ? ($totalReg / $hoursPerDay) : 0;
                $gross = $rate * $days;
            } elseif (in_array($rateType, ['bi-month', 'bimonth', 'bimonthly', 'semi-month', 'semimonth'])) {
                // Your old logic: assume 120 hours per cutoff for bi-monthly rates
                $hoursPerCutoff = 120;
                $hourly         = $hoursPerCutoff > 0 ? ($rate / $hoursPerCutoff) : 0;
                $gross          = $hourly * $totalReg;
            } elseif (in_array($rateType, ['hour', 'hourly'])) {
                // Kurt: 10 / hour → 10 * 24 = 240
                $gross = $rate * $totalReg;
            } else {
                // Fallback: treat as hourly if unknown type
                $gross = $rate * $totalReg;
            }
            ...
            $payroll[] = (object)[
                ...
                'days'             => $days,          // optional, for display if you like
                'gross'            => round($gross, 2)
            ];
        }
