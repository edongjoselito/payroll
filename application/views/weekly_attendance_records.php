...
<?php
// 🔐 role once for this view
$role = $this->session->userdata('position') ?: $this->session->userdata('level');
?>

<style>
thead th { position: sticky; top: 0; background-color: #fff; z-index: 2; }
td, th { vertical-align: middle !important; text-align: center; }
.btn i { margin-right: 0 !important; }
td { min-height: 40px; height: 40px; }
th:first-child, td:first-child {
  position: sticky; left: 0; background: #f8f9fa; z-index: 1;
  box-shadow: 2px 0 5px rgba(0,0,0,0.05);
}
.edit-attendance:hover { background-color: #f5f5f5; cursor: pointer; }
.toast-header.bg-success { background-color: #28a745 !important; }

/* Buttons */
.btn { border-radius: .375rem; font-size: 14px; padding: 8px 18px; font-weight: 500; transition: transform .5s ease; }
...
h4.page-title, h4.text-dark.font-weight-bold {
  ...
  border-left: 5px solid #007bff; padding-left: 10px; margin-bottom: 1rem;
}

/* Modals */
.modal-content { border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,.05); }
.modal-header { background-color: #f8f9fa; border-bottom: 1px solid #dee2e6; }
.modal-footer { border-top: 1px solid #dee2e6; }

/* Table */
.table thead th { font-size: 13px; font-weight: 600; background-color: #f1f3f5; color: #343a40; }
.table-striped tbody tr:nth-of-type(odd) { background-color: #fafafa; }

@media print {
  .btn, .modal, .modal-backdrop, .navbar, .sidebar, .page-title, .alert, .modal-open body { display: none !important; }
                          ...
                          <select class="form-control" id="view_payroll_month" name="payroll_month" required>
                            ...
                            <?php if (!empty($saved_months)): ?>
                              <?php foreach ($saved_months as $row): ?>
                                <option value="<?= $row->payroll_month ?>"><?= date('F Y', strtotime($row->payroll_month . '-01')) ?></option>
                              <?php endforeach; ?>
                            <?php else: ?>
                ...
                <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                  <form method="get" action="<?= base_url('WeeklyAttendance/records') ?>">
                    <div class="modal-content shadow-sm border-0">
                        ...
                        <div class="form-group">
                          ...
                          <select name="batchRange" id="attendanceBatch" class="form-control" required>
                            ...
                            <?php foreach ($attendance_periods as $batch): ?>
                              <option data-project="<?= $batch->projectID ?>" data-start="<?= $batch->start ?>" data-end="<?= $batch->end ?>">
                      ...
                      <div class="modal-footer">
                        <button type="submit" class="btn btn-success"><i class="mdi mdi-check"></i> View Records</button>
                        <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="mdi mdi-close-circle-outline"></i> Cancel</button>
                      </div>
                    </div>
                  </form>
                </div>
    ...
    <div class="card shadow mb-4">
      <div class="card-body">
        ...
        <?php foreach ($batches as $batch): ?>
          <?php
          if ($batch['projectID'] != $selectedProjectID || $batch['from'] != $selectedFrom || $batch['to'] != $selectedTo) { continue; }
          $project = $batch['project']; $projectID = $batch['projectID'];
          $from = $batch['from']; $to = $batch['to'];
          $dates = $batch['dates']; $attendances = $batch['attendances'];
          ?>
          <a id="batch-<?= $projectID ?>-<?= $from ?>-<?= $to ?>"></a>

          <h5 class="text-dark font-weight-bold mb-3">🗂 Attendance Batch: <?= date('F j', strtotime($from)) ?> - <?= date('F j, Y', strtotime($to)) ?></h5>

          <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="card shadow-sm mb-4">
              <div class="card-body">
                <h5 class="mb-1"><i class="mdi mdi-office-building text-info mr-1"></i><strong><?= $project->projectTitle ?></strong></h5>
                <p class="text-dark small mb-0"><?= date('F j, Y', strtotime($from)) ?> to <?= date('F j, Y', strtotime($to)) ?></p>
              </div>
            </div>
                ...
                <tr>
                  ...
                  <?php foreach ($dates as $d): ?>
                    ...
                    <td class="text-<?= $color ?> edit-attendance"
                        ...
                        data-overtime="<?= $overtimeHrs ?>" data-holiday="<?= $holidayHrs ?>"
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content shadow border-0">
      <div class="modal-header bg-danger text-white">
        <h5 class="modal-title" id="errorModalLabel"><i class="mdi mdi-alert-circle-outline mr-2"></i> Error</h5>
        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
      </div>
      <div class="modal-body text-dark">
        <div class="d-flex align-items-center">
          <i class="mdi mdi-alert mr-2 text-warning" style="font-size:24px;"></i>
          <div><strong>Some selected dates have no data.</strong><br>Only dates with existing attendance records will be shown.</div>
        </div>
      </div>
      <div class="modal-footer">
...
<div class="modal fade" id="noAttendanceModal" tabindex="-1" role="dialog" aria-labelledby="noAttendanceModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content border-0 shadow-sm">
      <div class="modal-header bg-danger text-white">
        <h5 class="modal-title" id="noAttendanceModalLabel"><i class="mdi mdi-alert-circle-outline mr-2"></i> No Attendance Found</h5>
        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
      </div>
      <div class="modal-body text-dark">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="mdi mdi-close-circle-outline"></i> Close</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="editAttendanceModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
...
<div aria-live="polite" aria-atomic="true" style="position:fixed; top:70px; left:50%; transform:translateX(-50%); z-index:1055;">
  <div class="toast show" id="successToast" role="alert" data-delay="3000" style="min-width:250px;">
    ...
    <div class="toast-body text-dark"><?= $this->session->flashdata('update_success'); ?></div>
  </div>
</div>
<?php endif; ?>
...
$(document).ready(function () {
  ...
  // Focus when filter modal opens (safe if modal is absent)
  if ($('#filterModal').length) {
    $('#filterModal').on('shown.bs.modal', function () { $('#project').focus(); });
  }

  // Attendance batch filtering (guard when modal is hidden)
  const projectSelect = document.getElementById('project');
  const batchSelect   = document.getElementById('attendanceBatch');

  if (projectSelect && batchSelect) {
